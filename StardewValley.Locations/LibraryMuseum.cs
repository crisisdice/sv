...
namespace StardewValley.Locations
{
	public class LibraryMuseum : GameLocation
	{
		...
		public List<Item> getRewardsForPlayer(Farmer who)
		{
			...
			if (!who.specialBigCraftables.Contains(1545) && museumContainsTheseItems(new int[2] { 108, 122 }, museumItems) && archItems > 10)
			{
			...
			if (!who.specialBigCraftables.Contains(1302) && museumContainsTheseItems(new int[2] { 583, 584 }, museumItems))
			{
				AddRewardIfUncollected(who, rewards, new Furniture(1302, Vector2.Zero));
			}
			if (!who.specialBigCraftables.Contains(1303) && museumContainsTheseItems(new int[2] { 580, 585 }, museumItems))
			{
