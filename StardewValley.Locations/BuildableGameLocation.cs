	...
	public class BuildableGameLocation : GameLocation
	{
		...
		public override bool isCollidingPosition(Microsoft.Xna.Framework.Rectangle position, xTile.Dimensions.Rectangle viewport, bool isFarmer, int damagesFarmer, bool glider, Character character, bool pathfinding, bool projectile = false, bool ignoreCharacterRequirement = false)
		{
			if (!glider && buildings.Count > 0)
			{
				...
				foreach (Building b in buildings)
				{
					...
					if (animal != null)
					{
						Microsoft.Xna.Framework.Rectangle door3 = b.getRectForAnimalDoor();
						door3.Height += 64;
						if (door3.Contains(position) && b.buildingType.Value.Contains(animal.buildingTypeILiveIn.Value))
						{
							continue;
						}
					}
		...
		public bool buildStructure(Building b, Vector2 tileLocation, Farmer who, bool skipSafetyChecks = false)
		{
			if (!skipSafetyChecks)
			{
				...
				{
					for (int x3 = 0; x3 < (int)b.tilesWide; x3++)
					{
				...
				{
					int x6 = additionalPlacementTile.X;
					int y6 = additionalPlacementTile.Y;
					pokeTileForConstruction(new Vector2(tileLocation.X + (float)x6, tileLocation.Y + (float)y6));
				}
				for (int y5 = 0; y5 < (int)b.tilesHigh; y5++)
				{
					for (int x4 = 0; x4 < (int)b.tilesWide; x4++)
					{
						Vector2 currentGlobalTilePosition = new Vector2(tileLocation.X + (float)x4, tileLocation.Y + (float)y5);
				...
				{
					int x5 = additionalPlacementTile2.X;
					int y4 = additionalPlacementTile2.Y;
					Vector2 currentGlobalTilePosition2 = new Vector2(tileLocation.X + (float)x5, tileLocation.Y + (float)y4);
			...
			if (b.indoors.Value != null)
			{
				...
				{
					if (w.TargetName == base.Name)
					{
						w.TargetX = b.humanDoor.X + (int)b.tileX;
						w.TargetY = b.humanDoor.Y + (int)b.tileY + 1;
					}
				}
			}
			...
			Action<Vector2> cleanup_tile = delegate(Vector2 tile_location)
			{
			...
			for (int y2 = 0; y2 < (int)b.tilesHigh; y2++)
			{
				...
				{
					cleanup_tile(new Vector2(tileLocation.X + (float)x, tileLocation.Y + (float)y2));
				}
			}
			foreach (Point additionalPlacementTile3 in b.additionalPlacementTiles)
			{
				int x2 = additionalPlacementTile3.X;
				int y = additionalPlacementTile3.Y;
				cleanup_tile(new Vector2(tileLocation.X + (float)x2, tileLocation.Y + (float)y));
			}
			return true;
		}
		...
		public bool buildStructure(BluePrint structureForPlacement, Vector2 tileLocation, Farmer who, bool magicalConstruction = false, bool skipSafetyChecks = false)
		{
			...
			{
			case "Stable":
				b = new Stable(GuidHelper.NewGuid(), structureForPlacement, tileLocation);
				break;
			case "Coop":
			case "Big Coop":
			case "Deluxe Coop":
