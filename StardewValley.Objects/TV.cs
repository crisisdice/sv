	...
	public class TV : Furniture
	{
		...
		private string[] getFishingInfo()
		{
			...
			foreach (KeyValuePair<int, string> v in dictionary)
			{
				...
				{
					if (i.Value.Split('/')[4 + currentSeasonNumber].Contains(v.Key.ToString() ?? "") && !locationsFound.Contains(getSanitizedFishingLocation(i.Key)))
					{
		...
		protected virtual string getTodaysTip()
		{
			...
			if (!tips.ContainsKey((Game1.stats.DaysPlayed % 224u).ToString() ?? ""))
			{
				return Game1.content.LoadString("Strings\\StringsFromCSFiles:TV.cs.13148");
			}
			return tips[(Game1.stats.DaysPlayed % 224u).ToString() ?? ""];
		}
		...
		protected virtual string[] getWeeklyRecipe()
		{
			...
			try
			{
				string recipeName2 = cookingRecipeChannel[whichWeek.ToString() ?? ""].Split('/')[0];
				text[0] = cookingRecipeChannel[whichWeek.ToString() ?? ""].Split('/')[1];
				if (CraftingRecipe.cookingRecipes.ContainsKey(recipeName2))
				{
					string[] split = CraftingRecipe.cookingRecipes[recipeName2].Split('/');
					text[1] = ((LocalizedContentManager.CurrentLanguageCode != 0) ? (Game1.player.cookingRecipes.ContainsKey(cookingRecipeChannel[whichWeek.ToString() ?? ""].Split('/')[0]) ? Game1.content.LoadString("Strings\\StringsFromCSFiles:TV.cs.13151", split[^1]) : Game1.content.LoadString("Strings\\StringsFromCSFiles:TV.cs.13153", split[^1])) : (Game1.player.cookingRecipes.ContainsKey(cookingRecipeChannel[whichWeek.ToString() ?? ""].Split('/')[0]) ? Game1.content.LoadString("Strings\\StringsFromCSFiles:TV.cs.13151", recipeName2) : Game1.content.LoadString("Strings\\StringsFromCSFiles:TV.cs.13153", recipeName2)));
				}
				else
				{
					text[1] = ((LocalizedContentManager.CurrentLanguageCode != 0) ? (Game1.player.cookingRecipes.ContainsKey(cookingRecipeChannel[whichWeek.ToString() ?? ""].Split('/')[0]) ? Game1.content.LoadString("Strings\\StringsFromCSFiles:TV.cs.13151", cookingRecipeChannel[whichWeek.ToString() ?? ""].Split('/').Last()) : Game1.content.LoadString("Strings\\StringsFromCSFiles:TV.cs.13153", cookingRecipeChannel[whichWeek.ToString() ?? ""].Split('/').Last())) : (Game1.player.cookingRecipes.ContainsKey(cookingRecipeChannel[whichWeek.ToString() ?? ""].Split('/')[0]) ? Game1.content.LoadString("Strings\\StringsFromCSFiles:TV.cs.13151", recipeName2) : Game1.content.LoadString("Strings\\StringsFromCSFiles:TV.cs.13153", recipeName2)));
				}
