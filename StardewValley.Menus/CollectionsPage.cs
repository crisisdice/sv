	...
	public class CollectionsPage : IClickableMenu
	{
			...
			: base(x, y, width, height)
		{
			...
			foreach (KeyValuePair<int, string> kvp2 in list)
			{
				...
				collections[whichCollection].Last().Add(new ClickableTextureComponent(kvp2.Key + " " + farmerHas2 + " " + farmerHasButNotMade, new Rectangle(xPos4, yPos4, 64, 64), null, "", Game1.objectSpriteSheet, Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, kvp2.Key, 16, 16), 4f, farmerHas2)
				{
			...
			foreach (string s in Game1.player.mailReceived)
			{
				if (mail.ContainsKey(s))
				{
					...
					string[] split = mail[s].Split(new string[1] { "[#]" }, StringSplitOptions.None);
			...
			else if (currentTab == 6)
			{
				if (secretNotesData != null)
				{
					...
					else
					{
						...
						string[] split2 = letter_text.Split(new string[1] { Environment.NewLine }, StringSplitOptions.None);
						int max_lines = 15;
