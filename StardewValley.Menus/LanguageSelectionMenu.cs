	...
	public class LanguageSelectionMenu : IClickableMenu
	{
		...
		public override void draw(SpriteBatch b)
		{
			...
			foreach (ClickableComponent c in languages)
			{
				...
				if (modLanguageLookup.ContainsKey(c.name))
				{
					ModLanguage language_data = modLanguageLookup[c.name];
					Texture2D mod_button_texture = Game1.temporaryContent.Load<Texture2D>(language_data.ButtonTexture);
