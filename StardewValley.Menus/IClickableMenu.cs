...
namespace StardewValley.Menus
{
	...
	public abstract class IClickableMenu
	{
		...
		public static void drawHoverText(SpriteBatch b, StringBuilder text, SpriteFont font, int xOffset = 0, int yOffset = 0, int moneyAmountToDisplayAtBottom = -1, string boldTitleText = null, int healAmountToDisplay = -1, string[] buffIconsToDisplay = null, Item hoveredItem = null, int currencySymbol = 0, int extraItemToShowIndex = -1, int extraItemToShowAmount = -1, int overrideX = -1, int overrideY = -1, float alpha = 1f, CraftingRecipe craftingIngredients = null, IList<Item> additional_craft_materials = null)
		{
			...
			if (moneyAmountToDisplayAtBottom > -1)
			{
				b.DrawString(font, moneyAmountToDisplayAtBottom.ToString() ?? "", new Vector2(x + 16, y + 16 + 4) + new Vector2(2f, 2f), Game1.textShadowColor);
				b.DrawString(font, moneyAmountToDisplayAtBottom.ToString() ?? "", new Vector2(x + 16, y + 16 + 4) + new Vector2(0f, 2f), Game1.textShadowColor);
				b.DrawString(font, moneyAmountToDisplayAtBottom.ToString() ?? "", new Vector2(x + 16, y + 16 + 4) + new Vector2(2f, 0f), Game1.textShadowColor);
				b.DrawString(font, moneyAmountToDisplayAtBottom.ToString() ?? "", new Vector2(x + 16, y + 16 + 4), Game1.textColor);
				switch (currencySymbol)
				{
				case 0:
					b.Draw(Game1.debrisSpriteSheet, new Vector2((float)(x + 16) + font.MeasureString(moneyAmountToDisplayAtBottom.ToString() ?? "").X + 20f, y + 16 + 16), Game1.getSourceRectForStandardTileSheet(Game1.debrisSpriteSheet, 8, 16, 16), Color.White, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, 0.95f);
					break;
				case 1:
					b.Draw(Game1.mouseCursors, new Vector2((float)(x + 8) + font.MeasureString(moneyAmountToDisplayAtBottom.ToString() ?? "").X + 20f, y + 16 - 5), new Rectangle(338, 400, 8, 8), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
					break;
				case 2:
					b.Draw(Game1.mouseCursors, new Vector2((float)(x + 8) + font.MeasureString(moneyAmountToDisplayAtBottom.ToString() ?? "").X + 20f, y + 16 - 7), new Rectangle(211, 373, 9, 10), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
					break;
				case 4:
					b.Draw(Game1.objectSpriteSheet, new Vector2((float)(x + 8) + font.MeasureString(moneyAmountToDisplayAtBottom.ToString() ?? "").X + 20f, y + 16 - 7), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 858, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
					break;
				}
				y += 48;
			}
