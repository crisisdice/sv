...
namespace StardewValley.Menus
{
	public class CharacterCustomization : IClickableMenu
	{
			...
			: base(Game1.uiViewport.Width / 2 - (632 + IClickableMenu.borderWidth * 2) / 2, Game1.uiViewport.Height / 2 - (648 + IClickableMenu.borderWidth * 2) / 2 - 64, 632 + IClickableMenu.borderWidth * 2, 648 + IClickableMenu.borderWidth * 2 + 64)
		{
			...
			shirtOptions = new List<int> { 0, 1, 2, 3, 4, 5 };
			hairStyleOptions = new List<int> { 0, 1, 2, 3, 4, 5 };
			accessoryOptions = new List<int> { 0, 1, 2, 3, 4, 5 };
			this.source = source;
			setUpPositions();
		...
		public virtual void LoadFarmTypeData()
		{
			...
			farmTypeHoverText.Add(Game1.content.LoadString("Strings\\UI:Character_FarmStandard"));
			farmTypeHoverText.Add(Game1.content.LoadString("Strings\\UI:Character_FarmFishing"));
			farmTypeHoverText.Add(Game1.content.LoadString("Strings\\UI:Character_FarmForaging"));
			farmTypeHoverText.Add(Game1.content.LoadString("Strings\\UI:Character_FarmMining"));
			farmTypeHoverText.Add(Game1.content.LoadString("Strings\\UI:Character_FarmCombat"));
			farmTypeHoverText.Add(Game1.content.LoadString("Strings\\UI:Character_FarmFourCorners"));
			farmTypeHoverText.Add(Game1.content.LoadString("Strings\\UI:Character_FarmBeach"));
			...
			if (farm_types != null)
			{
				foreach (ModFarmType farm_type in farm_types)
				{
					...
					farmTypeHoverText.Add(Game1.content.LoadString(farm_type.TooltipStringPath));
			...
			if (farm_types != null)
			{
				_farmPages = (int)Math.Floor((float)(farmTypeButtonNames.Count - 1) / 7f) + 1;
			}
		}
		...
		public virtual void RefreshFarmTypeButtons()
		{
			...
			if (index < farmTypeButtonNames.Count)
			{
				farmTypeButtons.Add(new ClickableTextureComponent(farmTypeButtonNames[index], new Rectangle(baseFarmButton.X, baseFarmButton.Y + 616, 88, 80), null, farmTypeHoverText[index], farmTypeIcons[index].Key, farmTypeIcons[index].Value, 4f)
				{
					myID = 546,
					upNeighborID = -99998,
					leftNeighborID = -99998,
			...
			{
				farmTypeNextPageButton = new ClickableTextureComponent("", new Rectangle(baseFarmButton.X + 64 + 8, baseFarmButton.Y + 352 + 12, 64, 64), null, "", Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 33), 1f)
				{
					myID = 547,
					upNeighborID = -99998,
					leftNeighborID = -99998,
		...
		private void optionButtonClick(string name)
		{
			if (name.StartsWith("ModFarm_"))
			{
				if (source == Source.NewGame || source == Source.HostNewFarm)
				{
					...
					foreach (ModFarmType farm_type in list)
					{
		...
		public override void performHoverAction(int x, int y)
		{
			...
			if (farmTypeNextPageButton != null)
			{
				farmTypeNextPageButton.tryHover(x, y);
			}
			if (farmTypePreviousPageButton != null)
			{
		...
		public override void draw(SpriteBatch b)
		{
			...
			foreach (ClickableTextureComponent c4 in petButtons)
			{
				if (c4.visible)
				{
					c4.draw(b);
					if ((c4.name.Equals("Cat") && Game1.player.catPerson) || (c4.name.Equals("Dog") && !Game1.player.catPerson))
					{
			...
			foreach (ClickableComponent c3 in labels)
			{
				...
				else if (c3 == shirtLabel)
				{
					...
					sub = ((int)Game1.player.shirt + 1).ToString() ?? "";
				}
				...
				{
					offset = 21f - Game1.smallFont.MeasureString(c3.name).X / 2f;
					sub = ((int)Game1.player.pants + 1).ToString() ?? "";
				}
			...
			if (farmTypeButtons.Count > 0)
			{
				...
				for (int i = 0; i < farmTypeButtons.Count; i++)
				{
					...
					int index = i + _currentFarmPage * 7;
			...
			if (hoverText != null && hoverText.Count() > 0)
			{
