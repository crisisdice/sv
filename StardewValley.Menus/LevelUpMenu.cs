		...
		public static void AddMissedLevelRecipes(Farmer farmer)
		{
			...
			foreach (int skill in skills)
			{
				for (int level = 0; level <= farmer.GetUnmodifiedSkillLevel(skill); level++)
				{
					...
					{
						string conditions2 = v2.Value.Split('/')[4];
						if (conditions2.Contains(Farmer.getSkillNameFromIndex(skill)) && conditions2.Contains(level.ToString() ?? "") && !farmer.craftingRecipes.ContainsKey(v2.Key))
						{
					...
					{
						string conditions = v.Value.Split('/')[3];
						if (conditions.Contains(Farmer.getSkillNameFromIndex(skill)) && conditions.Contains(level.ToString() ?? "") && !farmer.cookingRecipes.ContainsKey(v.Key))
						{
