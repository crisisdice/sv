...
namespace StardewValley.Menus
{
	public class LetterViewerMenu : IClickableMenu
	{
				...
				if (!isFromCollection)
				{
					...
					else if (split[1].Equals("craftingRecipe"))
					{
						...
						if (LocalizedContentManager.CurrentLanguageCode != 0)
						{
							...
							{
								learnedRecipe = craftingRecipes[learnedRecipe].Split('/')[^1];
							}
						}
					}
		...
		{
			int index = -1;
			for (index = text.IndexOf("["); index >= 0; index = text.IndexOf("[", index + 1))
			{
				...
				if (end_index >= 0)
				{
					...
					try
					{
						string[] split = text.Substring(index + 1, end_index - index - 1).Split(' ');
						if (split[0] == "letterbg")
						{
							if (split.Length == 2)
							{
						...
						else if (split[0] == "textcolor")
						{
							string color_string = split[1].ToLower();
							string[] color_lookup = new string[9] { "black", "blue", "red", "purple", "white", "orange", "green", "cyan", "gray" };
							customTextColor = -1;
