	...
	public class CraftingRecipe
	{
		...
		public virtual void drawRecipeDescription(SpriteBatch b, Vector2 position, int width, IList<Item> additional_crafting_items)
		{
			...
			for (int i = 0; i < recipeList.Count; i++)
			{
				...
				Utility.drawTinyDigits(recipeList.Values.ElementAt(i), b, new Vector2(position.X + 32f - Game1.tinyFont.MeasureString(recipeList.Values.ElementAt(i).ToString() ?? "").X, position.Y + 64f + (float)(i * 64 / 2) + (float)(i * 4) + 21f), 2f, 0.87f, Color.AntiqueWhite);
