...
namespace StardewValley
{
	public class Farmer : Character, IComparable
	{
		...
		public void performRenovation(string location_name)
		{
			...
			if (location != null && location is FarmHouse farmhouse)
			{
		...
		public Point getMailboxPosition()
		{
			...
			{
				if (b.isCabin && b.nameOfIndoors == homeLocation)
				{
		...
		public bool tryToCraftItem(List<int[]> ingredients, double successRate, int itemToCraft, bool bigCraftable, string craftingOrCooking)
		{
			...
			foreach (int[] ingredient in ingredients)
			{
				...
				{
					int[] cheapestIndex = new int[2] { 99999, -1 };
					for (int k = 0; k < items.Count; k++)
					{
						...
						{
							locationOfIngredients.Add(new int[2] { k, 1 });
							break;
						}
			...
			string text = craftingOrCooking.ToLower();
			if (!(text == "crafting"))
			{
				if (text == "cooking")
				{
		...
		public virtual void DrawUsername(SpriteBatch b)
		{
			...
			username = Game1.multiplayer.getUserName(UniqueMultiplayerID);
			if (username == null)
			{
				return;
			}
		...
		public override void MovePosition(GameTime time, xTile.Dimensions.Rectangle viewport, GameLocation currentLocation)
		{
			...
			if (CanMove || Game1.eventUp || controller != null)
			{
				...
				{
					Warp warp2 = Game1.currentLocation.isCollidingWithWarp(this.nextPosition(0), this);
					...
					{
						Microsoft.Xna.Framework.Rectangle tmp3 = this.nextPosition(0);
						tmp3.Width /= 4;
						bool leftCorner = currentLocation.isCollidingPosition(tmp3, viewport, isFarmer: true, 0, glider: false, this);
						tmp3.X += tmp3.Width * 3;
						bool rightCorner2 = currentLocation.isCollidingPosition(tmp3, viewport, isFarmer: true, 0, glider: false, this);
						if (leftCorner && !rightCorner2 && !currentLocation.isCollidingPosition(this.nextPosition(1), viewport, isFarmer: true, 0, glider: false, this))
						{
							position.X += (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
						}
						else if (rightCorner2 && !leftCorner && !currentLocation.isCollidingPosition(this.nextPosition(3), viewport, isFarmer: true, 0, glider: false, this))
						{
							position.X -= (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
						}
					}
				...
				if (movementDirections.Contains(2))
				{
					...
					{
						Microsoft.Xna.Framework.Rectangle tmp4 = this.nextPosition(2);
						tmp4.Width /= 4;
						bool leftCorner2 = currentLocation.isCollidingPosition(tmp4, viewport, isFarmer: true, 0, glider: false, this);
						tmp4.X += tmp4.Width * 3;
						bool rightCorner = currentLocation.isCollidingPosition(tmp4, viewport, isFarmer: true, 0, glider: false, this);
						if (leftCorner2 && !rightCorner && !currentLocation.isCollidingPosition(this.nextPosition(1), viewport, isFarmer: true, 0, glider: false, this))
						{
							position.X += (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
						}
						else if (rightCorner && !leftCorner2 && !currentLocation.isCollidingPosition(this.nextPosition(3), viewport, isFarmer: true, 0, glider: false, this))
						{
							position.X -= (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
						}
					}
				...
				if (movementDirections.Contains(1))
				{
					...
					else if (movementDirections.Count == 1)
					{
						...
						tmp2.Height /= 4;
						bool topCorner2 = currentLocation.isCollidingPosition(tmp2, viewport, isFarmer: true, 0, glider: false, this);
						tmp2.Y += tmp2.Height * 3;
						bool bottomCorner2 = currentLocation.isCollidingPosition(tmp2, viewport, isFarmer: true, 0, glider: false, this);
				...
				{
					Warp warp = Game1.currentLocation.isCollidingWithWarp(this.nextPosition(3), this);
					...
					else if (movementDirections.Count == 1)
					{
						...
						tmp.Height /= 4;
						bool topCorner = currentLocation.isCollidingPosition(tmp, viewport, isFarmer: true, 0, glider: false, this);
						tmp.Y += tmp.Height * 3;
						bool bottomCorner = currentLocation.isCollidingPosition(tmp, viewport, isFarmer: true, 0, glider: false, this);
