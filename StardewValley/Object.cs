...
namespace StardewValley
{
	...
	public class Object : Item
	{
		...
		/// <summary>
		/// constructor for big craftables
		/// </summary>
		...
		protected virtual string loadDisplayName()
		{
			...
			else
			{
				...
				if ((bool)bigCraftable)
				{
					...
					{
						return objectInformation2.Split('/')[^1];
					}
				}
				else
				{
			...
			if (dropInItem is Object)
			{
				...
				else if (name.Equals("Preserves Jar"))
				{
					...
					case 812:
					{
						...
						aged_roe = ((!(dropIn is ColoredObject colored_object)) ? new Object(447, 1) : new ColoredObject(447, 1, colored_object.color));
		...
		public virtual int GetRadiusForScarecrow()
		{
			...
			{
				if (context_tag != null && context_tag.StartsWith("crow_scare_radius_"))
				{
					string s = context_tag.Substring("crow_scare_radius".Length + 1);
					int radius = 0;
					if (int.TryParse(s, out radius))
					{
						return radius;
					}
				}
		...
		public override string checkForSpecialItemHoldUpMeessage()
		{
			...
			else if ((int)parentSheetIndex == 160)
			{
				...
				case 891:
				{
					...
					for (int j = x / 64 - 2; j <= x / 64 + 2; j++)
					{
						for (int l = y / 64 - 2; l <= y / 64 + 2; l++)
						{
							v2.X = j;
							v2.Y = l;
							if (location.terrainFeatures.ContainsKey(v2) && location.terrainFeatures[v2] is FruitTree)
							{
		...
		protected virtual float getPriceAfterMultipliers(float startPrice, long specificPlayerID = -1L)
		{
			...
			foreach (Farmer player in Game1.getAllFarmers())
			{
				...
				if (player.professions.Contains(20) && (((int)parentSheetIndex >= 334 && (int)parentSheetIndex <= 337) || (int)parentSheetIndex == 910))
				{
					multiplier *= 1.5f;
				}
