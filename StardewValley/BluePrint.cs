	...
	public class BluePrint
	{
		...
		public void drawDescription(SpriteBatch b, int x, int y, int width)
		{
			...
			foreach (KeyValuePair<int, int> kvp in itemsRequired)
			{
				...
				Utility.drawTinyDigits(kvp.Value, b, new Vector2((float)(x + 32) - Game1.tinyFont.MeasureString(kvp.Value.ToString() ?? "").X, (float)(yPosition + 32) - Game1.tinyFont.MeasureString(kvp.Value.ToString() ?? "").Y), 1f, 0.9f, Color.AntiqueWhite);
			...
			if (moneyRequired > 0)
			{
				...
				yPosition += (int)Game1.smallFont.MeasureString(moneyRequired.ToString() ?? "").Y;
			}
		}
	}
}
