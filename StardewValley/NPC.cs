	...
	public class NPC : Character, IComparable
	{
		...
		protected override string translateName(string name)
		{
			...
			default:
			{
				...
				{
					return dispositions[name].Split('/')[^1];
				}
				return name;
			}
			}
		...
		public virtual bool PathToOnFarm(Point destination, PathFindController.endBehavior on_path_success = null)
		{
			...
			if (path != null)
			{
				controller = new PathFindController(path, this, base.currentLocation);
		...
		private void engagementResponse(Farmer who, bool asRoommate = false)
		{
			...
			if (asRoommate && Game1.content.Load<Dictionary<string, string>>("Data\\EngagementDialogue").ContainsKey(base.Name + "Roommate0"))
			{
				...
				string attemptUniqueEngagementString2 = Game1.content.LoadStringReturnNullIfNotFound("Strings\\StringsFromCSFiles:" + base.Name + "_EngagedRoommate");
				if (attemptUniqueEngagementString2 != null)
				{
					CurrentDialogue.Push(new Dialogue(attemptUniqueEngagementString2, this));
				}
				else
				{
					attemptUniqueEngagementString2 = Game1.content.LoadStringReturnNullIfNotFound("Strings\\StringsFromCSFiles:" + base.Name + "_Engaged");
					if (attemptUniqueEngagementString2 != null)
					{
						CurrentDialogue.Push(new Dialogue(attemptUniqueEngagementString2, this));
					}
		...
		public virtual void tryToReceiveActiveObject(Farmer who)
		{
			...
			else
			{
				...
				else
				{
					...
					string safe_name = base.Name.ToLower().Replace(' ', '_');
					...
					else if ((who.friendshipData.ContainsKey(base.Name) && who.friendshipData[base.Name].GiftsThisWeek < 2) || (who.spouse != null && who.spouse.Equals(base.Name)) || this is Child || isBirthday(Game1.currentSeason, Game1.dayOfMonth))
					{
						...
						if ((bool)datable && who.spouse != null && who.spouse != base.Name && !who.hasCurrentOrPendingRoommate() && Utility.isMale(who.spouse) == Utility.isMale(base.Name) && Game1.random.NextDouble() < 0.3 - (double)((float)who.LuckLevel / 100f) - who.DailyLuck && !isBirthday(Game1.currentSeason, Game1.dayOfMonth) && who.friendshipData[base.Name].IsDating())
						{
							...
							spouse.CurrentDialogue.Push(new Dialogue(Game1.content.LoadString("Strings\\StringsFromCSFiles:NPC.cs.3985", displayName), spouse));
						}
					}
					else
					{
		...
		public virtual void performTenMinuteUpdate(int timeOfDay, GameLocation l)
		{
			...
			else
			{
				...
				if (!c.Name.Equals(base.Name) && !(c is Horse))
				{
					...
					if (dispositions.ContainsKey(base.Name) && !dispositions[base.Name].Split('/')[9].Split(' ').Contains(c.Name) && isFacingToward(c.getTileLocation()))
					{
						sayHiTo(c);
					}
				}
		...
		public void sayHiTo(Character c)
		{
			if (getHi(c.displayName) != null)
			{
				...
				if (c is NPC && Game1.random.NextDouble() < 0.66 && (c as NPC).getHi(displayName) != null)
				{
		...
		public string tryToGetMarriageSpecificDialogueElseReturnDefault(string dialogueKey, string defaultMessage = "")
		{
			...
			if (isRoommate())
			{
				try
				{
					if (Game1.content.Load<Dictionary<string, string>>("Characters\\Dialogue\\MarriageDialogue" + GetDialogueSheetName() + "Roommate") != null)
					{
						...
						marriageDialogues = Game1.content.Load<Dictionary<string, string>>("Characters\\Dialogue\\MarriageDialogue" + GetDialogueSheetName() + "Roommate");
			...
			if (isRoommate() && marriageDialogues != null && marriageDialogues.ContainsKey(dialogueKey + "Roommate"))
			{
				return marriageDialogues[dialogueKey + "Roommate"];
			}
			if (marriageDialogues != null && marriageDialogues.ContainsKey(dialogueKey))
			{
				return marriageDialogues[dialogueKey];
			...
			if (Dialogue != null && Dialogue.Count != 0)
			{
				...
				if (Game1.player.spouse != null && Game1.player.spouse == base.Name)
				{
					if (Game1.player.spouse.Equals(base.Name) && Game1.player.isEngaged())
					{
						...
						{
							currentDialogue.Push(new Dialogue(Game1.content.Load<Dictionary<string, string>>("Data\\EngagementDialogue")[base.Name + "Roommate" + r.Next(2)], this));
						}
				...
				{
					Dialogue.TryGetValue(idForClones.ToString() ?? "", out currentDialogueStr);
		...
		public bool checkForNewCurrentDialogue(int heartLevel, bool noPreface = false)
		{
			...
			if (Dialogue.ContainsKey(string.Concat(preface, Game1.currentLocation.name, "_", getTileX().ToString(), "_", getTileY().ToString())))
			{
				CurrentDialogue.Push(new Dialogue(Dialogue[string.Concat(preface, Game1.currentLocation.name, "_", getTileX().ToString(), "_", getTileY().ToString())], this)
				{
		...
		public void checkForMarriageDialogue(int timeOfDay, GameLocation location)
		{
			...
			case 1800:
				if (location is FarmHouse)
				{
					...
					suffix = ((which < 0) ? base.Name : (which.ToString() ?? ""));
					currentMarriageDialogue.Clear();
		...
		public void setNewDialogue(string dialogueSheetName, string dialogueSheetKey, int numberToAppend = -1, bool add = false, bool clearOnMovement = false)
		{
			...
			if (dialogueSheetName.Contains("Marriage"))
			{
				...
				{
					CurrentDialogue.Push(new Dialogue(tryToGetMarriageSpecificDialogueElseReturnDefault(dialogueSheetKey + ((numberToAppend != -1) ? (numberToAppend.ToString() ?? "") : "") + nameToAppend), this)
					{
						removeOnNextMove = clearOnMovement
					});
				}
			}
			else if (Game1.content.Load<Dictionary<string, string>>("Characters\\Dialogue\\" + dialogueSheetName).ContainsKey(dialogueSheetKey + ((numberToAppend != -1) ? (numberToAppend.ToString() ?? "") : "") + nameToAppend))
			{
				CurrentDialogue.Push(new Dialogue(Game1.content.Load<Dictionary<string, string>>("Characters\\Dialogue\\" + dialogueSheetName)[dialogueSheetKey + ((numberToAppend != -1) ? (numberToAppend.ToString() ?? "") : "") + nameToAppend], this)
				{
					removeOnNextMove = clearOnMovement
				});
			}
		...
		private List<string> getLocationRoute(string startingLocation, string endingLocation)
		{
			...
			{
				if (s.First().Equals(startingLocation, StringComparison.Ordinal) && s.Last().Equals(endingLocation, StringComparison.Ordinal) && ((int)gender == 0 || !s.Contains<string>("BathHouse_MensLocker", StringComparer.Ordinal)) && ((int)gender != 0 || !s.Contains<string>("BathHouse_WomensLocker", StringComparer.Ordinal)))
				{
					return s;
				}
			}
		...
		{
			Dictionary<string, string> room_data = Game1.content.Load<Dictionary<string, string>>("Data\\SpousePatios");
			if (room_data != null && room_data.ContainsKey(base.Name))
			{
				string[] split = room_data[base.Name].Split('/');
				if (split.Length > 2)
				{
					int[] frames = Utility.parseStringToIntArray(split[2]);
					Point offset = Point.Zero;
					if (split.Length > 3)
					{
						string[] offset_split = split[3].Split(' ');
						if (offset_split.Length > 1)
						{
							offset.X = int.Parse(offset_split[0]) * 4;
							offset.Y = int.Parse(offset_split[1]) * 4;
						}
					}
					drawOffset.Value = Utility.PointToVector2(offset);
					List<FarmerSprite.AnimationFrame> animation_frames = new List<FarmerSprite.AnimationFrame>();
					for (int i = 0; i < frames.Length; i++)
					{
						animation_frames.Add(new FarmerSprite.AnimationFrame(frames[i], 100, 0, secondaryArm: false, flip: false));
					}
					Sprite.setCurrentAnimation(animation_frames);
					return;
				}
			}
			switch ((string)name)
				...
				if (r.NextDouble() < 0.6 && !Game1.currentSeason.Equals("winter") && !hasSomeoneWateredCrops)
				{
					...
					if (!origin2.Equals(Vector2.Zero))
					{
						...
						for (int x2 = wateringArea2.X; x2 < wateringArea2.Right; x2++)
						{
							...
							{
								currentPosition2.X = x2;
								currentPosition2.Y = y2;
								if (farm.isTileOnMap(currentPosition2) && farm.terrainFeatures.ContainsKey(currentPosition2) && farm.terrainFeatures[currentPosition2] is HoeDirt && Game1.IsMasterGame && (farm.terrainFeatures[currentPosition2] as HoeDirt).needsWatering())
								{
									(farm.terrainFeatures[currentPosition2] as HoeDirt).state.Value = 1;
								}
							}
						}
						faceDirection(2);
					...
					if (!origin.Equals(Vector2.Zero))
					{
						...
						for (int x = wateringArea.X; x < wateringArea.Right; x++)
						{
							for (int y = wateringArea.Y; y < wateringArea.Bottom; y++)
							{
								currentPosition.X = x;
								currentPosition.Y = y;
								if (farm.isTileOnMap(currentPosition) && farm.objects.ContainsKey(currentPosition) && farm.objects[currentPosition] is Fence && Game1.IsMasterGame)
								{
									(farm.objects[currentPosition] as Fence).repair();
								}
							}
						}
						currentMarriageDialogue.Clear();
				...
				if (r.NextDouble() < 0.5)
				{
					...
					if (wall != null)
					{
						...
						case "Sebastian":
							...
							break;
						case "Haley":
							styles.AddRange(new int[7] { 1, 7, 10, 35, 49, 84, 99 });
							break;
						case "Abigail":
							styles.AddRange(new int[10] { 2, 13, 23, 26, 46, 45, 64, 77, 106, 107 });
							break;
						case "Leah":
							styles.AddRange(new int[7] { 44, 108, 43, 45, 92, 37, 29 });
							break;
						case "Alex":
							styles.AddRange(new int[1] { 6 });
							break;
						case "Shane":
							styles.AddRange(new int[3] { 6, 21, 60 });
							break;
						case "Krobus":
							styles.AddRange(new int[2] { 23, 24 });
							break;
						}
						if (styles.Count > 0)
						{
						...
						farmHouse.SetWallpaper(style.ToString(), wall);
		...
		{
			bool added = false;
			if (l != null && !route.Contains<string>(l.name, StringComparer.Ordinal))
			{
				route.Add(l.name);
