...
namespace StardewValley
{
	public class Farm : BuildableGameLocation, IAnimalLocation
	{
		...
		public override void DayUpdate(int dayOfMonth)
		{
			...
			{
				if (characters[l] is JunimoHarvester)
				{
			...
			{
				if (characters[m] is Monster && (characters[m] as Monster).wildernessFarmMonster)
				{
			...
			if (characters.Count > 5)
			{
				...
				{
					if (characters[i2] is GreenSlime && Game1.random.NextDouble() < 0.035)
					{
						characters.RemoveAt(i2);
						slimesEscaped++;
					}
				}
				if (slimesEscaped > 0)
				{
					Game1.multiplayer.broadcastGlobalMessage((slimesEscaped == 1) ? "Strings\\Locations:Farm_1SlimeEscaped" : "Strings\\Locations:Farm_NSlimesEscaped", false, slimesEscaped.ToString() ?? "");
				}
			}
			if (Game1.whichFarm == 5)
			{
			...
			if (ShouldSpawnForestFarmForage() && !Game1.IsWinter)
			{
				while (Game1.random.NextDouble() < 0.75)
				{
					...
					if (!isTileLocationTotallyClearAndPlaceable(v) || getTileIndexAt((int)v.X, (int)v.Y, "AlwaysFront") != -1 || ((Game1.whichFarm != 2 || !(v.X < 18f)) && !doesTileHavePropertyNoNull((int)v.X, (int)v.Y, "Type", "Back").Equals("Grass")))
					{
						continue;
					}
					int whichItem = 792;
				...
				if (objects.Count() > 0)
				{
					for (int n = 0; n < 6; n++)
					{
						Object o = objects.Pairs.ElementAt(Game1.random.Next(objects.Count())).Value;
			...
			{
				Vector2 pos = objKeys.ElementAt(k);
				if (!(terrainFeatures[pos] is HoeDirt) || (terrainFeatures[pos] as HoeDirt).crop != null)
				{
					continue;
				}
				if (objects.ContainsKey(pos))
				{
					Object tile_object = objects[pos];
					if (tile_object != null && tile_object.IsSpawnedObject && tile_object.isForage(this))
					{
						continue;
					}
				}
		...
		public void addCrows()
		{
			...
			{
				if ((bool)v3.Value.bigCraftable && v3.Value.IsScarecrow())
				{
		...
		public override Object getFish(float millisecondsAfterNibble, int bait, int waterDepth, Farmer who, double baitPotency, Vector2 bobberTile, string location = null)
		{
			...
			{
				string fish_location = getMapProperty("FarmFishLocationOverride");
				if (fish_location == "")
				{
				...
				{
					string[] fish_location_split = fish_location.Split(' ');
					try
					{
						_fishLocationOverride = fish_location_split[0];
						_fishChanceOverride = float.Parse(fish_location_split[1]);
					}
					catch (Exception)
					{
						_fishLocationOverride = "";
		...
		public void addSpouseOutdoorArea(string spouseName)
		{
			...
			string map_path = "spousePatios";
			if (room_data != null && room_data.ContainsKey(spouseName))
			{
				try
				{
					string[] array = room_data[spouseName].Split('/');
					map_path = array[0];
			...
			Map obj = Game1.game1.xTileContent.Load<Map>("Maps\\" + map_path);
			int columns = obj.Layers[0].LayerWidth / width;
			_ = obj.Layers[0].LayerHeight / height;
			...
			ApplyMapOverride(map_path, "spouse_patio", new Microsoft.Xna.Framework.Rectangle(mapReader.X, mapReader.Y, areaToRefurbish.Width, areaToRefurbish.Height), areaToRefurbish);
			bool spot_found = false;
			for (int x = areaToRefurbish.Left; x < areaToRefurbish.Right; x++)
			{
				for (int y = areaToRefurbish.Top; y < areaToRefurbish.Bottom; y++)
				{
					if (getTileIndexAt(new Point(x, y), "Paths") == 7)
					{
						spousePatioSpot = new Point(x, y);
						spot_found = true;
						break;
					}
				}
		...
		public override void pokeTileForConstruction(Vector2 tile)
		{
			...
			{
				if (npc is Pet && npc.getTileLocation() == tile)
				{
					Pet obj = npc as Pet;
					obj.FacingDirection = Game1.random.Next(0, 4);
					obj.faceDirection(obj.FacingDirection);
					obj.CurrentBehavior = 0;
					obj.forceUpdateTimer = 2000;
					obj.setMovingInFacingDirection();
				}
			}
		}
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (!hasSeenGrandpaNote)
			{
				...
				b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2((grandpa_shrine.X + 1) * 64, grandpa_shrine.Y * 64)), new Microsoft.Xna.Framework.Rectangle(575, 1972, 11, 8), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.044800997f);
			}
		}
		...
		public bool ShouldSpawnMountainOres()
		{
			if (!_mountainForageRectangle.HasValue)
			{
				...
				string[] split = getMapProperty("SpawnMountainFarmOreRect").Split(' ');
				if (split.Length == 4)
				{
					try
					{
						rectangle.X = int.Parse(split[0]);
						rectangle.Y = int.Parse(split[1]);
						rectangle.Width = int.Parse(split[2]);
						rectangle.Height = int.Parse(split[3]);
					}
					catch (Exception)
					{
						rectangle.X = 0;
				...
				_mountainForageRectangle = rectangle;
			}
