...
namespace StardewValley
{
	public class SaveGame
	{
		...
		public static XmlSerializer farmerSerializer = new XmlSerializer(typeof(Farmer), new Type[1] { typeof(Tool) });
		...
		public static void LoadFarmType()
		{
			...
			{
				int farm_type_value = 0;
				if (!int.TryParse(loaded.whichFarm, out farm_type_value))
				{
					Game1.whichFarm = -1;
					throw new Exception(loaded.whichFarm + " is not a valid farm type.");
				}
				Game1.whichFarm = farm_type_value;
			}
		}
		...
		public static IEnumerator<int> getLoadEnumerator(string file)
		{
			...
			while (current_save_fix < 35)
			{
				...
				{
					current_save_fix++;
					SaveFixes saveFixes = (SaveFixes)current_save_fix;
		...
		public static void loadDataToLocations(List<GameLocation> gamelocations)
		{
			...
			{
				if (l4 is Cellar && Game1.getLocationFromName(l4.name) == null)
				{
					Game1.locations.Add(new Cellar("Maps\\Cellar", l4.name));
				}
				if (l4 is FarmHouse)
				{
					GameLocation realLocation3 = Game1.getLocationFromName(l4.name);
					(realLocation3 as FarmHouse).setMapForUpgradeLevel((realLocation3 as FarmHouse).upgradeLevel);
					(realLocation3 as FarmHouse).fireplaceOn.Value = (l4 as FarmHouse).fireplaceOn;
					(realLocation3 as FarmHouse).fridge.Value = (l4 as FarmHouse).fridge;
					(realLocation3 as FarmHouse).farmerNumberOfOwner = (l4 as FarmHouse).farmerNumberOfOwner;
					(realLocation3 as FarmHouse).ReadWallpaperAndFloorTileData();
				}
				if (l4.name.Equals("Farm"))
				{
					GameLocation realLocation2 = Game1.getLocationFromName(l4.name);
					foreach (Building building in ((Farm)l4).buildings)
					{
						building.load();
					}
					((Farm)realLocation2).buildings.Set(((Farm)l4).buildings);
					foreach (FarmAnimal value in ((Farm)l4).animals.Values)
					{
						value.reload(null);
					}
					((Farm)realLocation2).greenhouseUnlocked.Value = ((Farm)l4).greenhouseUnlocked.Value;
					((Farm)realLocation2).greenhouseMoved.Value = ((Farm)l4).greenhouseMoved.Value;
					((Farm)realLocation2).UpdatePatio();
				}
				if (l4.name.Equals("MovieTheater"))
				{
					(Game1.getLocationFromName(l4.name) as MovieTheater).dayFirstEntered.Set((l4 as MovieTheater).dayFirstEntered);
				}
			}
			...
			foreach (GameLocation l3 in gamelocations)
			{
				...
				for (int k = l3.characters.Count - 1; k >= 0; k--)
				{
					initializeCharacter(l3.characters[k], realLocation);
				}
			...
			if (Game1.getLocationFromName("FarmCave") is FarmCave farm_cave)
			{
				farm_cave.UpdateReadyFlag();
			}
