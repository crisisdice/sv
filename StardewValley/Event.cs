	...
	public class Event
	{
		...
		public virtual void command_fork(GameLocation location, GameTime time, string[] split)
		{
			if (split.Length > 2)
			{
				...
				{
					string[] newCommands2 = ((split.Length <= 3) ? Game1.content.Load<Dictionary<string, string>>("Data\\Events\\" + Game1.currentLocation.Name)[split[2]].Split('/') : Game1.content.LoadString(split[2]).Split('/'));
					eventCommands = newCommands2;
					CurrentCommand = 0;
					forked = !forked;
				}
				else
			...
			{
				string[] newCommands = (isFestival ? festivalData[split[1]].Split('/') : Game1.content.Load<Dictionary<string, string>>("Data\\Events\\" + Game1.currentLocation.Name)[split[1]].Split('/'));
		...
		{
			string[] newCommands = ((!isFestival) ? Game1.content.Load<Dictionary<string, string>>("Data\\Events\\" + Game1.currentLocation.Name)[split[1]].Split('/') : festivalData[split[1]].Split('/'));
			eventCommands = newCommands;
			CurrentCommand = 0;
			eventSwitched = true;
		}
		...
		public virtual void command_quickQuestion(GameLocation location, GameTime time, string[] split)
		{
			if (!Game1.isQuestion && Game1.activeClickableMenu == null)
			{
				...
				string[] questionAndAnswerSplit = obj.Substring(obj.IndexOf(' ') + 1).Split(new string[1] { "(break)" }, StringSplitOptions.None)[0].Split('#');
		...
		public bool tryToLoadFestival(string festival)
		{
			...
			string obj = festivalData["conditions"].Split('/')[0];
			...
			if (!obj.Equals(Game1.currentLocation.Name) || Game1.timeOfDay < startTime || Game1.timeOfDay >= endTime)
			{
				return false;
			}
			int year_count = 1;
			while (festivalData.ContainsKey("set-up_y" + year_count + 1))
			{
				year_count++;
			}
		...
		private void populateWalkLocationsList()
		{
			...
			for (int j = 2; j < eventCommands.Length; j++)
			{
				...
				if (split[0] == "move" && split[1].Equals("farmer"))
				{
			...
			foreach (NPC k in actors)
			{
				...
				for (int i = 2; i < eventCommands.Length; i++)
				{
					...
					if (split2[0] == "move" && split2[1].Equals(k.Name))
					{
		...
		private void trySpecialSetUp(GameLocation location)
		{
			...
			case 4324303:
			{
				...
				for (int j = 0; j < eventCommands.Length; j++)
				{
					...
					{
						string[] split = eventCommands[j].Split(' ');
						split[1] = (int.Parse(split[1]) - 26 + bed_spot2.X).ToString() ?? "";
						split[2] = (int.Parse(split[2]) - 13 + bed_spot2.Y).ToString() ?? "";
						...
						{
							eventCommands[j] = string.Join(" ", split);
						}
					}
				}
				break;
			...
			case 3912132:
			{
				...
				bed_spot.X--;
				if (!location.isTileLocationTotallyClearAndPlaceable(Utility.PointToVector2(bed_spot) + new Vector2(-2f, 0f)))
				{
					bed_spot.X++;
				}
				...
				for (int i = 0; i < eventCommands.Length; i++)
				{
					...
					{
						string[] split2 = eventCommands[i].Split(' ');
						split2[1] = (int.Parse(split2[1]) - 26 + bed_spot.X).ToString() ?? "";
						split2[2] = (int.Parse(split2[2]) - 13 + bed_spot.Y).ToString() ?? "";
						...
						{
							eventCommands[i] = string.Join(" ", split2);
						}
					}
				}
				break;
		...
		private void addSpecificTemporarySprite(string key, GameLocation location, string[] split)
		{
			...
			case "BoatParrotSquawk":
			{
				...
				temporaryAnimatedSprite2.sourceRect.X = 24;
				temporaryAnimatedSprite2.sourceRectStartingPos.X = 24f;
			...
			case "getEndSlideshow":
			{
				...
				string[] s = obj.getEndSlideshow().Split('/');
		...
		private void setUpSecretSantaCommands()
		{
			...
			for (int i = 0; i < eventCommands.Length; i++)
			{
				...
				eventCommands[i] = eventCommands[i].Replace("warpX", secretSantaX.ToString() ?? "");
				eventCommands[i] = eventCommands[i].Replace("warpY", secretSantaY.ToString() ?? "");
		...
		public void draw(SpriteBatch b)
		{
			...
			if (isFestival && festivalData["file"] == "fall16")
			{
		...
		public void forceFestivalContinue()
		{
			...
			Game1.exitActiveMenu();
			string[] newCommands = GetFestivalDataForYear("mainEvent").Split('/');
			eventCommands = newCommands;
			CurrentCommand = 0;
			eventSwitched = true;
			playerControlSequence = false;
		...
		public void setUpFestivalMainEvent()
		{
			...
			string[] newCommands = rawFestivalData.Split('/');
			eventCommands = newCommands;
		}
			...
			if (isFestival)
			{
				...
				case 511:
					...
					if (!festivalShops.ContainsKey("starTokenShop"))
					{
						...
						stock2.Add(new Furniture(1307, Vector2.Zero), new int[2] { 100, 1 });
						stock2.Add(new Hat(19), new int[2] { 500, 1 });
						stock2.Add(new Object(Vector2.Zero, 110), new int[2] { 800, 1 });
						if (!Game1.player.mailReceived.Contains("CF_Fair"))
						{
							stock2.Add(new Object(434, 1), new int[2] { 2000, 1 });
						}
						stock2.Add(new Furniture(2488, Vector2.Zero), new int[2] { 500, 1 });
						switch (new Random((int)Game1.uniqueIDForThisGame + Game1.year * 17 + 19).Next(5))
						{
						case 0:
							stock2.Add(new Object(253, 1), new int[2] { 400, 1 });
							break;
						case 1:
							stock2.Add(new Object(215, 1), new int[2] { 250, 1 });
							break;
						case 2:
							stock2.Add(new Ring(888), new int[2] { 1000, 1 });
							break;
						case 3:
							stock2.Add(new Object(178, 100), new int[2] { 500, 1 });
							break;
						case 4:
							stock2.Add(new Object(770, 24), new int[2] { 1000, 1 });
							break;
						}
				...
				else if (isFestival)
				{
					if (who.IsLocalPlayer)
					{
						foreach (NPC k in actors)
						{
							...
							if (who.spouse != null && k.Name.Equals(who.spouse) && !festivalData["file"].Equals("spring24"))
							{
								if (k.isRoommate() && festivalData.ContainsKey(k.Name + "_roommate"))
								{
									k.CurrentDialogue.Clear();
									k.CurrentDialogue.Push(new Dialogue(GetFestivalDataForYear(k.Name + "_roommate"), k));
								}
								else if (festivalData.ContainsKey(k.Name + "_spouse"))
								{
									k.CurrentDialogue.Clear();
		...
		public void answerDialogue(string questionKey, int answerChoice)
		{
			...
			{
				string obj = eventCommands[Math.Min(eventCommands.Length - 1, CurrentCommand)];
				string[] newCommands = obj.Substring(obj.IndexOf(' ') + 1).Split(new string[1] { "(break)" }, StringSplitOptions.None)[1 + answerChoice].Split('\\');
				List<string> tmp = eventCommands.ToList();
