...
namespace StardewValley.Menus
{
  public class CharacterCustomization : IClickableMenu
  {
    ...
    public CharacterCustomization(List<int> shirtOptions, List<int> hairStyleOptions, List<int> accessoryOptions, bool wizardSource = false)
    ...
    private void optionButtonClick(string name)
    {
      ...
      if (stringHash <= 1367651536U)
      {
        ...
        {
          if ((int) stringHash != 1485672)
          {
            if ((int) stringHash == 989237149 && name == "Wilderness" && !this.wizardSource)
            {
              Game1.whichFarm = 4;
              Game1.spawnMonstersAtNight = true;
            }
          }
          else if (name == "Standard" && !this.wizardSource)
          {
            Game1.whichFarm = 0;
            Game1.spawnMonstersAtNight = false;
          }
        }
        else if ((int) stringHash != 1216165616)
        {
          if ((int) stringHash != 1265483177)
          {
            if ((int) stringHash == 1367651536 && name == "Riverland" && !this.wizardSource)
            {
              Game1.whichFarm = 1;
              Game1.spawnMonstersAtNight = false;
            }
          }
          else if (name == "Dog" && !this.wizardSource)
            Game1.player.catPerson = false;
        }
        else if (name == "Male" && !this.wizardSource)
        {
          Game1.player.changeGender(true);
          Game1.player.changeHairStyle(0);
        }
      }
      else if (stringHash <= 2246359087U)
      {
        if ((int) stringHash != 1761538983)
        {
          if ((int) stringHash == -2048608209 && name == "OK")
          {
        ...
        else if (name == "Cat" && !this.wizardSource)
          Game1.player.catPerson = true;
      }
      else if ((int) stringHash != -1791187840)
      {
        if ((int) stringHash != -1786556165)
        {
          if ((int) stringHash == -660443975 && name == "Female" && !this.wizardSource)
          {
            Game1.player.changeGender(false);
            Game1.player.changeHairStyle(16);
          }
        }
        else if (name == "Hills" && !this.wizardSource)
        {
          Game1.whichFarm = 3;
          Game1.spawnMonstersAtNight = false;
        }
      }
      else if (name == "Forest" && !this.wizardSource)
      {
        Game1.whichFarm = 2;
        Game1.spawnMonstersAtNight = false;
      }
