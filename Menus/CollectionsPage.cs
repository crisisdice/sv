  ...
  public class CollectionsPage : IClickableMenu
  {
      ...
      : base(x, y, width, height, false)
    {
      ...
      foreach (KeyValuePair<int, string> keyValuePair in Game1.objectInformation)
      {
        ...
        ClickableTextureComponent textureComponent9 = new ClickableTextureComponent(keyValuePair.Key.ToString() + " " + drawShadow.ToString(), new Rectangle(x1, y1, Game1.tileSize, Game1.tileSize), (string) null, "", Game1.objectSpriteSheet, Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, keyValuePair.Key, 16, 16), (float) Game1.pixelZoom, drawShadow);
      ...
      foreach (KeyValuePair<int, string> achievement in Game1.achievements)
      {
        ...
        {
          int x1 = num27 + numArray[5] % num29 * (Game1.tileSize + 4);
          int y1 = num28 + numArray[5] / num29 * (Game1.tileSize + 4);
          this.collections[5][0].Add(new ClickableTextureComponent(achievement.Key.ToString() + " " + flag.ToString(), new Rectangle(x1, y1, Game1.tileSize, Game1.tileSize), (string) null, "", Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 25, -1, -1), 1f, false));
          ++numArray[5];
        }
      }
    }
    ...
    {
      string str1 = listOfAchievementNumbers;
      char[] chArray = new char[1]{ ' ' };
      foreach (string str2 in str1.Split(chArray))
      {
        if (!Game1.player.achievements.Contains(Convert.ToInt32(str2)))
          return false;
      }
      return true;
    }
