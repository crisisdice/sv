...
namespace StardewValley.Menus
{
  public class DialogueBox : IClickableMenu
  {
    ...
    public DialogueBox(string dialogue, List<Response> responses, int width = 1200)
    {
      ...
      int y = this.y - (this.heightForQuestions - this.height) + SpriteText.getHeightOfString(this.getCurrentString(), width) + Game1.pixelZoom * 12;
      for (int index = 0; index < responses.Count; ++index)
      {
        List<ClickableComponent> responseCc = this.responseCC;
        ClickableComponent clickableComponent = new ClickableComponent(new Rectangle(this.x + Game1.pixelZoom * 2, y, width, SpriteText.getHeightOfString(responses[index].responseText, width) + Game1.pixelZoom * 4), "");
    ...
    private void setUpQuestions()
    {
      ...
      if (this.responses == null || !Game1.options.SnappyMenus)
        return;
      this.responseCC = new List<ClickableComponent>();
      int y = this.y - (this.heightForQuestions - this.height) + SpriteText.getHeightOfString(this.getCurrentString(), this.width) + Game1.pixelZoom * 12;
      for (int index = 0; index < this.responses.Count; ++index)
      {
        List<ClickableComponent> responseCc = this.responseCC;
        ClickableComponent clickableComponent = new ClickableComponent(new Rectangle(this.x + Game1.pixelZoom * 2, y, this.width - Game1.pixelZoom * 2, SpriteText.getHeightOfString(this.responses[index].responseText, this.width) + Game1.pixelZoom * 4), "");
        clickableComponent.myID = index;
          ...
          if (this.characterIndexInDialogue >= this.getCurrentString().Length - 1)
          {
            ...
            for (int index = 0; index < this.responses.Count; ++index)
            {
              if (index == this.selectedResponse)
                IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(375, 357, 3, 3), this.x + Game1.pixelZoom, y - Game1.pixelZoom * 2, this.width - Game1.pixelZoom * 2, SpriteText.getHeightOfString(this.responses[index].responseText, this.width) + Game1.pixelZoom * 4, Color.White, (float) Game1.pixelZoom, false);
