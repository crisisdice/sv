...
namespace StardewValley.Menus
{
  public class Fish
  {
    ...
    {
      this.animationTimer = this.animationTimer - time.ElapsedGameTime.Milliseconds;
      ...
      {
        this.rotation = this.rotation + (float) ((double) Game1.random.Next(-(int) this.dartingRandomness, (int) this.dartingRandomness) * Math.PI / 100.0);
        this.targetSpeed = this.rotation;
        this.dartingExtraSpeed = this.dartingIntensity / 20f;
        this.dartingExtraSpeed = this.dartingExtraSpeed * (float) (1.0 + (double) Game1.random.Next(-10, 10) / 100.0);
      ...
      {
        this.dartingTimer = this.dartingTimer - (float) time.ElapsedGameTime.Milliseconds;
        ...
        if (!this.isDarting)
          this.dartingExtraSpeed = this.dartingExtraSpeed - this.dartingExtraSpeed * 0.0005f * (float) time.ElapsedGameTime.Milliseconds;
      }
      ...
      if ((double) Math.Abs(this.rotation - this.targetRotation) > (double) Math.Abs(this.targetRotation / (100f - this.turnSpeed)))
        this.rotation = this.rotation + this.targetRotation / (100f - this.turnSpeed);
      this.rotation = this.rotation % 6.283185f;
      this.currentSpeed = this.currentSpeed + (float) (((double) this.targetSpeed - (double) this.currentSpeed) / 10.0);
    ...
    {
      b.Draw(Game1.mouseCursors, this.position + positionOfFishingField, new Rectangle?(new Rectangle(561, 1846 + this.indexOfAnimation * 16, 16, 16)), Color.White, this.rotation + 1.570796f, new Vector2(8f, 8f), 4f, SpriteEffects.None, 0.5f);
    }
  }
}
