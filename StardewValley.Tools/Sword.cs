...
namespace StardewValley.Tools
{
	public class Sword : Tool
	{
		...
		public void DoFunction(GameLocation location, int x, int y, int facingDirection, int power, Farmer who)
		{
			...
			{
				Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(352, Game1.random.Next(50, 120), 2, 1, new Vector2(areaOfEffect.Center.X - 32, areaOfEffect.Center.Y - 32) + new Vector2(Game1.random.Next(-32, 32), Game1.random.Next(-32, 32)), flicker: false, Game1.random.NextDouble() < 0.5));
			}
			string soundToPlay = "";
			if (!dontdestroyObjects)
			{
				if (location.objects.ContainsKey(tileLocation1) && !location.Objects[tileLocation1].Name.Contains("Stone") && !location.Objects[tileLocation1].Name.Contains("Stick") && !location.Objects[tileLocation1].Name.Contains("Stump") && !location.Objects[tileLocation1].Name.Contains("Boulder") && !location.Objects[tileLocation1].Name.Contains("Lumber") && !location.Objects[tileLocation1].IsHoeDirt)
				{
					if (location.Objects[tileLocation1].Name.Contains("Weed"))
					{
						...
						soundToPlay = ((location.Objects[tileLocation1].Category != -2) ? "cut" : "stoneCrack");
