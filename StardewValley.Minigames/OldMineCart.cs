...
namespace StardewValley.Minigames
{
	public class OldMineCart : IMinigame
	{
				...
				if (speedAccumulator >= (float)tileSize)
				{
					...
					if (distanceToTravel == -1 || totalMotion < distanceToTravel)
					{
						...
						if (noiseValue < -0.5)
						{
							...
							trackToAdd = ((!foundTrack) ? new Point(Game1.random.Next(1, 3), (Game1.random.NextDouble() < 0.5 && currentTrackY < 6) ? currentTrackY : (currentTrackY + 1)) : new Point(0, 999));
						}
						else
						{
		...
		public void draw(SpriteBatch b)
		{
			...
			for (int k = 0; k < lakeDecor.Count; k++)
			{
				b.Draw(texture, new Vector2((float)(k * tileSize) - lakeSpeedAccumulator, lakeDecor.ElementAt(k).Y * tileSize), new Rectangle(32 + lakeDecor.ElementAt(k).X * tileSize, 0, 16, 16), (lakeDecor.ElementAt(k).X == 0) ? midBGTint : lakeTint);
			}
			for (int n = 0; n <= screenWidth / 96 + 2; n++)
			{
				b.Draw(texture, new Vector2(0f - backBGPosition + (float)(n * 96), tileSize * 2), new Rectangle(64, 162, 96, 111), backBGTint);
			}
			for (int i4 = 0; i4 < screenWidth / 96 + 2; i4++)
			{
				b.Draw(texture, new Vector2(0f - midBGPosition + (float)(i4 * 96), 0f), new Rectangle(64, 0, 96, 162), midBGTint);
			}
			for (int i3 = 0; i3 < track.Count; i3++)
			{
				if (track.ElementAt(i3).X != 0)
				{
					b.Draw(texture, new Vector2(0f - speedAccumulator + (float)(i3 * tileSize), (track.ElementAt(i3).Y + ytileOffset) * tileSize - tileSize), new Rectangle(160 + (track.ElementAt(i3).X - 1) * 16, 144, 16, 32), trackTint);
					float darkness = 0f;
					for (int j2 = track.ElementAt(i3).Y + 1; j2 < screenHeight / tileSize; j2++)
					{
						b.Draw(texture, new Vector2(0f - speedAccumulator + (float)(i3 * tileSize), (j2 + ytileOffset) * tileSize), new Rectangle(16 + ((j2 % 2 == 0) ? ((track.ElementAt(i3).X + 1) % 2) : (track.ElementAt(i3).X % 2)) * 16, 32, 16, 16), trackTint);
						b.Draw(texture, new Vector2(0f - speedAccumulator + (float)(i3 * tileSize), (j2 + ytileOffset) * tileSize), new Rectangle(16 + ((j2 % 2 == 0) ? ((track.ElementAt(i3).X + 1) % 2) : (track.ElementAt(i3).X % 2)) * 16, 32, 16, 16), trackShadowTint * darkness);
						darkness += 0.1f;
					}
				}
			}
			for (int i2 = 0; i2 < obstacles.Count; i2++)
			{
				switch (obstacles[i2].X)
				{
				case 1:
					b.Draw(texture, new Vector2(0f - speedAccumulator + (float)(i2 * tileSize), (obstacles[i2].Y + ytileOffset) * tileSize), new Rectangle(16, 0, 16, 16), Color.White);
					break;
				case 2:
					b.Draw(Game1.debrisSpriteSheet, new Vector2(0f - speedAccumulator + (float)(i2 * tileSize), (obstacles[i2].Y + ytileOffset) * tileSize), Game1.getSourceRectForStandardTileSheet(Game1.debrisSpriteSheet, 8), Color.White, 0f, new Vector2(32f, 0f), 0.25f, SpriteEffects.None, 0f);
					break;
				}
			}
