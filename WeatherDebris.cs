...
namespace StardewValley
{
  public class WeatherDebris
  {
    ...
    public void update(bool slow)
    {
      ...
      {
        if ((double) this.position.X < (double) (-Game1.tileSize - Game1.tileSize / 4))
        {
          this.position.X = (float) Game1.viewport.Width;
          this.position.Y = (float) Game1.random.Next(0, Game1.viewport.Height - Game1.tileSize);
        }
        if ((double) this.position.Y > (double) (Game1.viewport.Height + Game1.tileSize / 4))
        {
        ...
        else if ((double) this.position.Y < (double) -Game1.tileSize)
        {
      ...
      switch (this.which)
      {
        ...
        case 2:
        case 3:
          this.animationTimer = this.animationTimer - Game1.currentGameTime.ElapsedGameTime.Milliseconds;
          if (this.animationTimer > 0)
            break;
          this.animationTimer = 100 + this.animationIntervalOffset;
          this.animationIndex = this.animationIndex + this.animationDirection;
          ...
          if (this.animationIndex > 10)
          {
            if (Game1.random.NextDouble() < 0.82)
            {
              ...
              this.dx = this.dx + 0.1f;
              this.dy = this.dy - 0.2f;
            }
            else
